...
class DrawState:
    ...
    def append(self, value: str) -> None:
        match value:
            ...
            case "LAYOUT_SEPARATOR":
                self.current_doc = self.current_doc + AlwaysLineBreak()
            case "LAYOUT_END":
                old_document = self.documents.pop()
                self.current_doc = old_document + Indent(
...
def gen_MeguKin(lark: lark.lark.Lark, start: Optional[str] = "top"):
    expected = {
        "LAYOUT_START": st.just("LAYOUT_START"),
        "LAYOUT_SEPARATOR": st.just("LAYOUT_SEPARATOR"),
        "LAYOUT_END": st.just("LAYOUT_END"),
    }
    return from_lark(grammar=lark, start=start, explicit=expected)
